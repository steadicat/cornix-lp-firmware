...
mod cornix_lp_peripheral {
    ...
    #[controller(poll)]
    fn cornix_indicator() {
        let spi = Spim::new_txonly_nosck(p.SPI3, IndicatorIrqs, p.P0_13, spim_config());
        let power = output_high_drive_low(p.P0_24);
        CornixIndicator::new(spi, power, Side::Peripheral)
    }
}
