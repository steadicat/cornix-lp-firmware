...
fn create_normal_unilateral_keyboard() -> Keyboard<'static, 1, 5, 2> {
    let hand = [[Hand::Left, Hand::Left, Hand::Right, Hand::Right, Hand::Right]];
    create_morse_keyboard(
        BehaviorConfig {
            morse: MorsesConfig {
...
fn create_normal_layer_tap_slash_keyboard() -> Keyboard<'static, 1, 2, 2> {
    ...
    let per_key_config: &'static mut PositionalConfig<1, 2> = Box::leak(Box::new(PositionalConfig::default()));
    Keyboard::new(wrap_keymap(keymap, per_key_config, behavior_config))
}

fn create_normal_layer_tap_d_keyboard() -> Keyboard<'static, 1, 2, 2> {
