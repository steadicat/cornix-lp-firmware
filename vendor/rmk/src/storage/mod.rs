...
mod tests {
    ...
    fn behavior_config_round_trip_preserves_vial_tap_hold_settings() {
        ...
        let storage_data = StorageData::BehaviorConfig(BehaviorConfig {
            prior_idle_time: stored_flow_tap_term(&behavior),
            morse_default_profile: behavior.morse.default_profile,
            combo_timeout: behavior.combo.timeout.as_millis() as u16,
            one_shot_timeout: behavior.one_shot.timeout.as_millis() as u16,
    ...
    fn build_hash_mismatch_preserves_storage() {
        block_on(async {
            ...
            store_item(
                ...
                &mut cache,
                &mut buffer,
                &(StorageKeys::LayoutConfig as u32),
            ...
            let stored_layout = fetch_item::<u32, StorageData, _>(
                &mut storage.flash,
                storage.storage_range.clone(),
                &mut NoCache::new(),
                &mut read_buffer,
            ...
            .unwrap()
            .unwrap();
            let stored_config = fetch_item::<u32, StorageData, _>(
                &mut storage.flash,
                storage.storage_range.clone(),
                &mut NoCache::new(),
                &mut read_buffer,
