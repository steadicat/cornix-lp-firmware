    ...
    Keyboard<'a, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
        ...
        for (pos, decision) in decisions {
            ...
            match decision {
                ...
                HeldKeyDecision::Release => {
                    ...
                    // Only fire buffered normal keys after the releasing key actually resolved.
    ...
    fn has_same_hand_unresolved_morse_before_normal(&self, released_pos: KeyboardEventPos) -> bool {
        ...
        let hand_info = &keymap.positional_config.hand;
        let released_hand = Self::get_hand(hand_info, released_key_pos);
        if released_hand == Hand::Unknown {
            return false;
        }
        ...
        self.held_buffer
            ...
            .any(|normal_key| {
                self.held_buffer.keys.iter().any(|morse_key| {
                    ...
                    let morse_hand = Self::get_hand(hand_info, morse_key_pos);
    ...
    async fn process_combo(&mut self, key_action: &KeyAction, event: KeyboardEvent) -> (Option<KeyAction>, bool) {
        ...
        if event.pressed && key_action.is_morse() {
            ...
            let tap_hold_is_combo_component = self
                .keymap
                .borrow()
                .behavior
                .combo
                ...
                .any(|combo| {
                    combo.config.layer.map_or(true, |layer| layer == current_layer)
                        && combo.config.actions.contains(key_action)
                });
    ...
    fn update_osl_after_key(&mut self) {
        match self.osl_state {
            OneShotState::Initial(l) => self.osl_state = OneShotState::Held(l),
            OneShotState::Single(layer_num) => {
                self.keymap.borrow_mut().deactivate_layer(layer_num);
